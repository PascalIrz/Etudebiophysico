...
model_normal_non_transformées_aspt <- glm(`ASPT`~`O2dissous`+`NH4`+`NO3`+`Ptot`,
                                          data= df_global_sans_stations,
                                          family = gaussian(link = "identity")) #si on rajoute le pH hypothèses vérifiées, si loi Gamma sans pH hypothèses validées pour cette loi
...
model_normal_non_transformées_pol <- glm(`polyvolI2M2`~`O2dissous`+`NH4`+`NO3`+`Ptot`
                                         ,data= df_global_sans_stations, family = gaussian(link = "identity")) #problème des notes = à 0
...
#################################################################################
#                                               Richesse taxonomique - test (ne fonctionne pas comme les autres)
#################################################################################

model_normal_non_transformées_ov <- glm(`richesI2M2`~`O2dissous`+`NH4`+`NO3`+`Ptot`+pH, data= df_global_sans_stations, family = gaussian(link = "identity")) 
summary(model_normal_non_transformées_ov)
model_simplifié <- MASS::stepAIC(model_normal_non_transformées_ov)
...
model_normal_non_transformées_ibd <- glm(`IBD`~ NH4+`NO3`+`Ptot`+ O2dissous, data= df_global_sans_stations, family = gaussian(link = "identity"))
