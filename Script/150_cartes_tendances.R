...
create_trend_map <- function(data_abondance_relative,
  ...
  data_sf <- data_sf %>%
    mutate(
      symbole = case_when(
        ...
        TRUE ~ "\u25CF" # cercle pour aucun changement
      ),
      couleur = case_when(
  ...
  map_plot <- ggplot() +
    ...
    geom_sf(data = data_sf) + # Ceci trace les points de base
    geom_sf_text(
      data = data_sf,
...
# Chironomes (Cd_Taxon_norm == "807") --> quelques baisses à l'ouest et augmentation à l'est
...
# Gammaridae (Cd_Taxon_norm %in% c("892","888","887")) --> pas de tendance 
...
# Asemmidae (Cd_Taxon_norm == "880") --> qu'une baisse en Ille et vilaine 
