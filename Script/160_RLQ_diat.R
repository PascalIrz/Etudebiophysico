...
Q_data_diat <- carayon %>%
  mutate(libelle_appel_taxon = trimws(as.character(libelle_taxon))) %>% 
  left_join(carayon_map, by = "libelle_appel_taxon") %>% 
  filter(!is.na(code_appel_taxon)) %>% 
  distinct(code_appel_taxon, .keep_all = TRUE) %>%
  as.data.frame() %>%
  column_to_rownames("code_appel_taxon") 
...
L_final_diat <- L_final_diat[, !sapply(L_final_diat, function(x) var(x, na.rm = TRUE) == 0 || all(is.na(x))), drop = FALSE] 
L_final_diat <- L_final_diat[rowSums(is.na(L_final_diat)) < ncol(L_final_diat), , drop = FALSE] 

Q_final_diat <- Q_final_diat[, !sapply(Q_final_diat, function(x) length(unique(x)) == 1), drop = FALSE] 
...
s.label(stations_scores_rlq_diat, clab = 0.8,
        xlim = c(min(stations_scores_rlq_diat[,1])*1.2, max(stations_scores_rlq_diat[,1])*1.2), 
...
taxons_avec_libelles_diat <- dplyr::left_join(
  df_codes_pour_jointure_diat,
  referentiel_nettoye_diat,
  by = "code_taxon_net_diat"
)

liste_libelles_diat <- unique(taxons_avec_libelles_diat$libelle_taxon)

if (length(liste_libelles_diat) > 0) {
  liste_libelles_sans_na_diat <- na.omit(liste_libelles_diat)
...
df_to_export_taxons_pression_diat <- data.frame(LibelleTaxon = liste_libelles_sans_na_diat)
write.xlsx(df_to_export_taxons_pression_diat, file = "Taxons_associes_pressions_diat.xlsx", rowNames = FALSE)


# Traits
traits_pression_diat <- carayon_weights[carayon_weights[,1] < seuil_bas_trait_diat, ] # Correction ici
if (nrow(traits_pression_diat) > 0) {
  traits_pression_sorted_diat <- traits_pression_diat[order(traits_pression_diat[,1], decreasing = TRUE),]
...
if (nrow(taxons_bonne_qualite_diat) > 0) {
  taxons_bonne_qualite_sorted_diat <- taxons_bonne_qualite_diat[order(taxons_bonne_qualite_diat[,1], decreasing = FALSE),]
  print(taxons_bonne_qualite_sorted_diat)
} else {
  print("Aucun taxon ne dépasse le seuil pour la bonne qualité sur cet axe.")
}

good_taxons_affiches_diat <- rownames(taxons_bonne_qualite_sorted_diat)

good_taxons_nets_diat <- tolower(trimws(good_taxons_affiches_diat))
df_codes_pour_jointure_good_diat <- data.frame(
  good_taxon_net = good_taxons_nets_diat
)

referentiel_nettoye_good_diat <- abondance_taxo_diat_filtre %>%
  mutate(
    good_taxon_net = tolower(trimws(code_appel_taxon))
  ) %>%
  select(good_taxon_net,libelle_appel_taxon)

good_taxons_avec_libelles_diat <- dplyr::left_join(
  df_codes_pour_jointure_good_diat,
  referentiel_nettoye_good_diat,
  by = "good_taxon_net"
)

liste_libelles_good_diat <- unique(good_taxons_avec_libelles_diat$libelle_taxon)

if (length(liste_libelles_good_diat) > 0) {
  liste_libelles_sans_na_good_diat <- na.omit(liste_libelles_good_diat)
...
df_to_export_bqualite_diat <- data.frame(LibelleTaxon = liste_libelles_sans_na_good_diat) 
write.xlsx(df_to_export_bqualite_diat, file = "Taxons_associes_bonne_qualite_diat.xlsx", rowNames = FALSE)


# Traits
traits_bonne_qualite_diat <- carayon_weights_diat[carayon_weights_diat[,1] > seuil_haut_trait_diat, ]
if (nrow(traits_bonne_qualite_diat) > 0) {
  traits_bonne_qualite_sorted_diat <- traits_bonne_qualite_diat[order(traits_bonne_qualite_diat[,1], decreasing = FALSE),]
  print(traits_bonne_qualite_sorted_diat)
} else {
  print("Aucun trait ne dépasse le seuil pour la bonne qualité sur cet axe.")
}

df_to_export_traits_bqualite_diat <- data.frame(Trait = rownames(traits_bonne_qualite_sorted_diat))
write.xlsx(df_to_export_traits_bqualite_diat, file = "Traits_associes_bonne_qualite_diat.xlsx", rowNames = FALSE)
  
  ############## Nutriments ####################

nutrients_names_diat <- c("1339", "1340", "1433","1335", "1350")


# Extraire les poids des nutriments
nutrients_weights_diat <- env_weights_diat[nutrients_names_diat, ] 
print(nutrients_weights_diat)

# Calculer la "direction moyenne" des nutriments sur chaque axe.
mean_nutrient_direction_Ax1_diat <- mean(nutrients_weights_diat[,1]) 
...
if (abs(mean_nutrient_direction_Ax1_diat) >= abs(mean_nutrient_direction_Ax2_diat)) { 
  ...
  chosen_axis_diat <- 2
  mean_nutrient_direction_diat <- mean_nutrient_direction_Ax2_diat 
  cat("L'Axe 2 est choisi comme axe principal de la dégradation par les nutriments (diatomées).\n")
}

seuil_bas_station_diat <- quantile(stations_scores_rlq_diat[, 1], probs = 0.25, na.rm = TRUE)
...
degraded_stations_diat <- stations_scores_rlq_diat[
  stations_scores_rlq_diat[,chosen_axis_diat] < 0 & 
    stations_scores_rlq_diat[,chosen_axis_diat] < seuil_bas_station_diat, ] 

# Tri par score croissant (les plus négatifs sont ceux qui sont les plus "bas" sur l'axe négatif)
degraded_stations_sorted_diat <- degraded_stations_diat[order(degraded_stations_diat[,chosen_axis_diat], decreasing = FALSE), ] 
message_no_station_diat <- paste0("Aucune station n'a de score entre 0 et -", seuil_bas_station_diat, " sur l'Axe ", chosen_axis_diat, " (diatomées).") 
...
taxons_degraded_conditions_sorted_diat <- taxons_degraded_conditions_diat[order(taxons_degraded_conditions_diat[,chosen_axis_diat], decreasing = FALSE), ] 
message_no_taxon_diat <- paste0("Aucun taxon n'a de score entre 0 et -", seuil_bas_taxon_diat, " sur l'Axe ", chosen_axis_diat, " (diatomées).") 
...
bad_taxons_affiches_diat <- rownames(taxons_degraded_conditions_sorted_diat) 

bad_taxons_nets_diat <- tolower(trimws(bad_taxons_affiches_diat)) 

df_codes_pour_jointure_bad_diat <- data.frame( 
  bad_taxon_net = bad_taxons_nets_diat
)

referentiel_nettoye_bad_diat <- abondance_taxo_diat_filtre %>% 
  mutate(
    bad_taxon_net = tolower(trimws(code_appel_taxon))
  ) %>%
  select(bad_taxon_net,libelle_appel_taxon)

bad_taxons_avec_libelles_diat <- dplyr::left_join( 
  df_codes_pour_jointure_bad_diat,
  referentiel_nettoye_bad_diat, 
  by = "bad_taxon_net"
)

liste_libelles_bad_diat <- unique(bad_taxons_avec_libelles_diat$libelle_taxon) 

if (length(liste_libelles_bad_diat) > 0) {
  liste_libelles_sans_na_bad_diat <- na.omit(liste_libelles_bad_diat) 
...
df_to_export_bad_taxons_diat <- data.frame(LibelleTaxon = liste_libelles_sans_na_bad_diat) 
...
traits_degraded_conditions_diat <- carayon_weights[ 
  carayon_weights[,chosen_axis_diat] < 0 &
    carayon_weights[,chosen_axis_diat] < seuil_bas_trait_diat, ] 
traits_degraded_conditions_sorted_diat <- traits_degraded_conditions_diat[order(traits_degraded_conditions_diat[,chosen_axis_diat], decreasing = FALSE), ] 
message_no_trait_diat <- paste0("Aucun trait n'a de poids entre 0 et -", seuil_bas_trait_diat, " sur l'Axe ", chosen_axis_diat, " (diatomées).") 
...
scaling_factor_viz_diat <- max(max_abs_score_station_diat, max_abs_score_taxon_diat) / max_abs_weight_combined_diat * 0.8

par(mar = c(5, 5, 4, 2) + 0.1)
plot(stations_scores_rlq_diat, type = "n", asp = 1, # Renommé
     xlim = range(stations_scores_rlq_diat[,1], taxon_scores_diat[,1], env_weights_diat[,1] * scaling_factor_viz_diat, carayon_weights_diat[,1] * scaling_factor_viz_diat)*1.1, 
     ylim = range(stations_scores_rlq_diat[,2], taxon_scores_diat[,2], env_weights_diat[,2] * scaling_factor_viz_diat, carayon_weights_diat[,2] * scaling_factor_viz_diat)*1.1, 
     xlab = paste0("Axe RLQ 1 (", round(rlq_result_diat$eig[1]/sum(rlq_result_diat$eig)*100, 2), "%)"), 
     ylab = paste0("Axe RLQ 2 (", round(rlq_result_diat$eig[2]/sum(rlq_result_diat$eig)*100, 2), "%)"), 
     main = "RLQ: Stations, Taxons, Traits & Nutriments (Diatomées)")

# 1. Ajouter les Stations
points(stations_scores_rlq_diat, pch = 20, col = "blue", cex = 0.8)

# 2. Ajouter les Taxons
points(taxon_scores_diat, pch = 3, col = "darkgreen", cex = 0.8) 

# 3. Ajouter les Paramètres Environnementaux, en surlignant les nutriments
for (i in 1:nrow(env_weights_diat)) { 
  col_arrow <- "darkgrey" 
  lwd_arrow <- 1
  cex_label <- 0.7
  if (rownames(env_weights_diat)[i] %in% nutrients_names_diat) { 
    col_arrow <- "darkred" 
    lwd_arrow <- 2
    cex_label <- 0.9 
  }
  arrows(0, 0, env_weights_diat[i,1] * scaling_factor_viz_diat, env_weights_diat[i,2] * scaling_factor_viz_diat, 
         length = 0.1, angle = 20, col = col_arrow, lwd = lwd_arrow)
  text(env_weights_diat[i,1] * scaling_factor_viz_diat * 1.1, env_weights_diat[i,2] * scaling_factor_viz_diat * 1.1, 
       labels = rownames(env_weights_diat)[i], cex = cex_label, col = col_arrow) 
}

# 4. Ajouter les Traits (vecteurs violets)
arrows(0, 0, carayon_weights_diat[,1] * scaling_factor_viz_diat, carayon_weights_diat[,2] * scaling_factor_viz_diat, 
       length = 0.1, angle = 20, col = "purple", lwd = 1.5)
text(carayon_weights_diat[,1] * scaling_factor_viz_diat * 1.1, carayon_weights_diat[,2] * scaling_factor_viz_diat * 1.1, 
     labels = rownames(carayon_weights_diat), cex = 0.7, col = "purple")
