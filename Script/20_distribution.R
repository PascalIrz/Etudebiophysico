...
ggplot(clean_minv_distrib, aes(x = resultat_indice, fill = classe_I2M2)) +
  ...
  geom_vline(
    xintercept = c(0.148, 0.295, 0.433, 0.665),
    linetype = "dashed",
    color = "grey50",
    linewidth = 0.8
  ) +
  ...
  facet_wrap( ~ libelle_indice,
              ...
              scales = "free") +
  labs(title = "Distribution des valeurs des métriques de l'I2M2 par classe de qualité",
...
# Dans le bon ordre
i2m2_class_boundaries$classe <-
  factor(i2m2_class_boundaries$classe, levels = names(class_colors_map))

# Pour chacune des facet
all_indices <- unique(clean_minv$code_indice)
background_rects_data <-
  do.call(rbind, lapply(all_indices, function(idx) {
    data.frame(code_indice = idx,
               i2m2_class_boundaries)
  }))

background_rects_data$code_indice <-
  factor(background_rects_data$code_indice, levels = levels(factor(clean_minv$code_indice)))
...
clean_ibd_distrib$classe_ibd <- factor(
  clean_ibd_distrib$classe_ibd,
  levels = c("Très bon", "Bon", "Moyen", "Médiocre", "Mauvais")
)
...
ggplot(clean_ibd_distrib, aes(x = resultat_indice, fill = classe_ibd)) +
  ...
  facet_wrap( ~ libelle_indice,
              labeller = label_wrap_gen(width = 20),
              scales = "free") +
  labs(title = "Distribution des valeurs des indices IBD et IPS par classe de qualité",
...
ibd_class_boundaries$classe <-
  factor(ibd_class_boundaries$classe, levels = names(ibd_class_colors_map))

# Pour chaque indice
all_ibd_indices <- unique(clean_ibd$code_indice)
background_rects_data_ibd <-
  do.call(rbind, lapply(all_ibd_indices, function(idx) {
    data.frame(code_indice = idx,
               ibd_class_boundaries)
  }))

background_rects_data_ibd$code_indice <-
  factor(background_rects_data_ibd$code_indice, levels = levels(factor(clean_ibd$code_indice)))
