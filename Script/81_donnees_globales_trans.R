...
periode_param <- tibble::tibble(
  ...
  periode = c("annuelle", "annuelle", "annuelle", "annuelle","annuelle", "annuelle","annuelle", "annuelle", "annuelle","annuelle","annuelle", "mars_decembre", "mars_decembre","juillet_septembre")
)
...
mean_physico_periode <- mean_physico %>%
  ...
  filter(
    (periode == "annuelle") |
      ...
      (periode == "juillet_septembre" & mois >= 7 & mois <= 9)
  )
...
save(
  ...
  i2m2_wide,
  physico_wide,
  file = "Data/81_donnees_globales_trans.rda"
)
