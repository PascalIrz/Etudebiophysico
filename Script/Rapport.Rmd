...
dernières données, 59% des masses d’eau sont concernés par les
...
ntre Nitrates et autres formes de l’azote, PTOT Micro-polluants ? Parler
...
aquatiques. Elle constitue un cadre destiné à harmoniser la gestion et
la protection des eaux européennes. Les états membres sont dans
l’obligation d’évaluer la qualité écologique de leurs masses d’eau et de
restaurer ou maintenir leur bon état écologique d’ici 2027. Les éléments
de qualité biologiques (EQB) retenues sont : les poissons, les
macrophytes, les macro-invertébrés et les diatomées. Cette évaluation de
l’état d’un milieu est permise grâce à une classifications qui
distingues les cours d’eau de références, définis comme ceux présentant
les caractéristiques attendues en l’absence de pression humaines
(concept de « conditions de référence » ; Bailey et al. 1998 et les
cours d’eau altérés. La notion d’écart à un état de référence est
intégrée dans le calcul des indices biologiques, et par conséquent dans
l’évaluation de l’état écologique des masses d’eau, ils seront alors
traduits en ratios de qualités biologiques. Les stations sont classées
de la manière suivante : réseau de référence pérenne (RRP), réseau de
contrôle de surveillance (RCS), réseau de contrôle opération (RCO). En
Bretagne, les trois-quarts des masses d’eau sont déclassés en raison de
leur mauvaise qualité biologique. Les diatomées sont le deuxième groupe
de bio-indicateur déclassant en Bretagne, suivi par les
...
La bioévaluation mène à la détermination de l'état d'un milieux par le
biais d'un indicateur biologique. Ce dernier désigne les espèces
biologiques ou animales qui constituent un indice précoce de
modifications biotiques ou abiotiques de l'environnement dues à des
activités humaines (<https://eau-grandsudouest.fr/glossaire/b>).
...
: la qualité de l’eau et la dégradation de l’habitat. L’I2M2 prend en
compte les caractéristiques taxonomiques et les traits biologiques des
communautés de macroinvertébrés benthiques, avec une détermination des
individus jusqu’au genre (et non plus seulement à la famille, ce qui
était le cas avec l’IBGN). De plus, il intègre les abondances des genres
recensés. L’échantillonnage est réalisé selon la norme NF T 90-333 dans
l’AFNOR, 2016.
...
A noter que les deux premières métriques traduisent davantage une
qualité de l'habitat et les autres une qualité des masses d'eau.
...
l’Ille-et-Vilaine (2% en bon état). A l’ouest de la Bretagne, la pente
et la diversité morphologique des cours d’eau améliore nettement leur
résilience. Le déficit pluviométrique en Ille-et-Vilaine induit un
écoulement, une capacité de dilution et d’autoépuration plus faible. De
plus, ce dernier est fortement urbanisé, ce qui implique un usage de
l’eau d’autant plus important. Les pressions majoritaires responsables
du non atteinte du bon état écologique de l’eau en Bretagne sont : la
dégradation de la morphologie des cours d’eau et l’intensification de
l’agriculture laitière au sud de Rennes.
...
période hivernale, où les pluies sont plus intenses. Le phosphore en
provenance d’industrie ou autre activité urbaine, est relargué toute
l’année, ce qui exerce une pression continue sur les cours d’eau. Cet
apport devient prédominant en période sèche. Un léger gradient de
...
élevé depuis 6 ans, c’est-à-dire 32,1 mg.L (percentile 90) pour les
bassins bretons. Elle a tout de même diminué de 17% depuis 1995 à
l’échelle régional. La mesure de la concentration en nitrates est
complétée d’une mesure débitmétrique car plus les débits sont importants
plus les quantités en azote nitrique sortant du bassin versant sont
élevées. Les formes de l’azote les plus toxiques sont les moins
oxygénées et peuvent être le produit du cycle de l’azote, elles feront
donc parties des espèces chimiques étudiées.
...
IBD au sein des différentes stations. DIRE QU'ON VEUT FAIRE UNE ETUDE
SPATIALE
...
majorité de ces stations se trouvent à l'est de la région , d'où
l'intérêt d'avoir une approche spatiale lors de l'étude.
...
diatomiques et macroinvertébrés, les éléments physico-chimiques retenues
sont les suivants :
...
Après chargement des données complètes "macro-invertébrés", seules les
...
été retenues pour l'analyse. En effet, des valeurs manquantes peuvent
être présentes en raison de la fréquence de prélèvement qui n'est pas la
même selon le réseau ou d'une impossibilité de réalisation du
prélèvement à cause des conditions hydrologiques. Au terme de cela, 112
stations constituent le jeu de données "macroinvertébré" et 233 stations
pour les diatomées.
...
valeurs sera possible. Il s'avère que les résultats confirme cette
suppostion.
...
tendances non linéaires, et ne nécessite pas de distributions normales.

### Réalisation d'une "fenêtre glissante"

Les prélèvements physico-chimiques sont réalisés tout au long de
l'année, parfois plusieurs fois par mois. Le facteur temporel étant
négligé dans notre analyse, les résultats seront agrégés. Cette méthode
permet de choisir la période temporelle sur laquelle calculer une
moyenne pour chacun des paramètres. Les critères sur lesquels se base
cette sélection sont :
...
-   réalisation d'un dataframe (df), avec toutes les combinaisons de
    mois-début et mois-fin pour chaque paramètre (soit 78 combinaisons
    possibles).

-   df avec les colonnes suivantes : *code_station_hydrobio,
    code_paramètre, annee, mois.*
...
Une première analyse permet d'évaluer le lien bivarié entre les indices
et les métriques, les paramètres physico-chimiques deux à deux et pour
terminer entre les indices et les variables environnementales. Une
matrice des corrélation est réalisée, calculant les corrélations entre
les indices et métriques puis entre paramètres physico-chimqiues, grâce
au test de corrélation de Spearman. Ce test est robuse en cas de non
normalité des distributions des variables et de relations non linéaires
entre elles. Dans un second temps, une autre représentation est choisie
pour étudier plus facilement l'indépendance des variables entre elles,
utile pour la réalisation du modèle.

### Analyses multivariées

L'analyse des composantes principales est une méthode statistique
...
été calculée pour chaque variable sur l’ensemble de la période d’étude.
Ainsi, chaque station est représentée par une seule valeur moyenne par
paramètre, indice ou métrique, permettant une analyse synthétique
inter-stations. Le choix de la moyenne comme statistique est justifié
par une volonté d'étudier des patterns généraux à l'échelle de chaque
station.

Pour chaque modèle, la fonction stepAIC sera appliqué pour retenir
...
chaque métrique pour faciliter la lecture. D'après les résultats, la
majorité des notes tendent vers le "Bon" et "Très Bon" état selon la
DCE. La plupart des métriques suivent une distribution simlaire à celle
de l'I2M2, légèrement symétrique, à l'exception de la richesse
taxonomique qui suit une distribution normale. Les métriques
d'ovoviviparité et de polyvoltinisme prennent de multiple fois une
valeur nulle, ce qui témoigne d'une mauvaise qualité des masses d'eau à
ces stations (qui se trouvent en majorité en Ille-et-Vilaine).
...
inter-stations seulement, en aggrégeant les valeurs. Le même
comportement est observé pour les indices IBD et IPS.
...
qu'une moyenne pour réprésenter des phénomènes généraux au à l'échelle
des stations.
...
variabilité temporelle limitée. Peu de tendances significatives sont
constatées pour les métriques de l'indice I2M2, notamment la richesse
taxonomique qui est la seule métrique qui ne croît dans aucune station.
...
environnementales et donc pressions étudiées.
...
variables explicatives afin d'éviter notamment la multicolinéarité.

En analysant la structure interne des variables biologiques, les
métriques qui réagissent davantage à la dégradation qualité de l'habitat
sont les moins corrélées à l'I2M2. De plus, elles contribuent très peu à
la construction de l'axe F1, dont les variables les plus contributives
sont l'I2M2, le polyvoltinisme, l'ASPT et l'ovoviviparité. Elles sont
donc peu représentée dans cet axe. D'après [SOURCE], à l'échelle
européenne c'était la diversité de Shannon et la richesse taxonomique
qui étaient les métriques les plus contributives de la note de l'I2M2.
