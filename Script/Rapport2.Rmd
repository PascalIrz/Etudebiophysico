...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width= 3, fig.height= 3, fig.cap= " Fig.7 : Réseaux des stations étudiées"  }
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width= 13, fig.height=15, fig.cap= " Fig.7 : Chroniques des prélèvements de diatomées pour le suivi DCE" }
...
```{r,echo=FALSE, message=FALSE, warning=FALSE, fig.width= 10, fig.height= 5, fig.cap= " Fig.10 : Distribution inter-annuelle de l'IBD et l'IPS, de 2008 à 2023" }
...
clean_minv_distrib <- clean_minv %>%
  mutate(
    classe_I2M2 = case_when(
      ...
      TRUE ~ NA_character_ # Gérer les NA si des resultat_indice sont manquants
    )
  ) %>%
  # Convertir 'classe_I2M2' en facteur avec l'ordre désiré pour la légende et les couleurs
  mutate(
    classe_I2M2 = factor(classe_I2M2,
                         levels = c("Très bon", "Bon", "Moyen", "Médiocre", "Mauvais"))
  )

# 2. Création de l'histogramme amélioré pour l'I2M2
ggplot(clean_minv_distrib, aes(x = resultat_indice, fill = classe_I2M2)) +
  # Utilisez 'binwidth' au lieu de 'bins'.
  # J'ai choisi 0.05, mais vous pouvez l'ajuster en fonction de la granularité désirée.
  # Une plus petite valeur rendra les barres plus étroites et la transition plus nette.
  geom_histogram(binwidth = 0.01, color = "black", alpha = 0.7, stat = "bin") +
  
  # Ajout de lignes verticales pour marquer les seuils des catégories
...
# 1. Préparation des données (identique à la précédente, car elle est correcte)
...
# 2. Création de l'histogramme avec une 'binwidth' plus fine
ggplot(clean_ibd_distrib, aes(x = resultat_indice, fill = classe_ibd)) +
  # Utilisez 'binwidth' au lieu de 'bins'.
  # Ajustez la valeur de 'binwidth' (ex: 0.5, 0.2, 0.1) selon l'échelle de vos données.
  # Une plus petite valeur rendra les barres plus étroites et la transition plus nette.
  geom_histogram(binwidth = 0.1, color = "black", alpha = 0.7, stat = "bin") +
  
  # Ajout des lignes verticales pour marquer les seuils des catégories
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width= 5, fig.height= 5, fig.cap= " Fig.12 : Carte des tendances temporelles de l'indice I2M2 en Bretagne : Ilona Garcia"}
...
i2m2_ibd_matrix <- i2m2_ibd %>% 
  cor(method="spearman")
corrplot.mixed(i2m2_ibd_matrix,upper="ellipse", tl.col = "black", # Couleur du texte des labels (noir)
               tl.cex = 1.2)
...
ggplot() +
  ...
  coord_sf(xlim = c(-5.2, -1), ylim = c(47, 49), expand = FALSE) +
  labs(title = "Etat chimique selon les concentrations en nitrates à partir des P90, données de 2015 à 2023",
       color = "Classe d'état") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
ggplot() +
  ...
  annotation_scale(location = "br", width_hint = 0.3) +
  coord_sf(xlim = c(-5.2, -1), ylim = c(47, 49), expand = FALSE) +
  labs(title = "-	Etat chimique selon les concentrations en phosphore total à partir des P90, données de 2015 à 2023)",
       color = "Classe d'état") +
  theme_minimal() +
  theme(legend.position = "bottom")
