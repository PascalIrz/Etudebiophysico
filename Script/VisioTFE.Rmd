...
-   Macro-invertébrés : code_station_hydrobio, libelle_station, code_indice (I2M2, ASPT, richesse taxonomique, indice de Shannon, polyvoltinisme, ovoviviparité), libelle_indice, resultat_indice, date_prelevement, code_qualification, longitute, latitude.
-   Diatomées : code_station_hydrobio, libelle_station, code_indice (IBD, IPS), libelle_indice, resultat_indice, date_prelevement, code_qualification, longitute, latitude.
-   Paramètres physico-chimiques : code_station_hydrobio, libelle_station, code_parametre, libelle_parametre resultat, code_support, code_fraction, code_qualification, date_prelevement.
...
mapview(aspt_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution ASPT")
mapview(ovi_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution Ovoviviparité ")
mapview(pol_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution polyvoltinisme")
mapview(sha_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution Indice de Shannon")
mapview(ric_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution Richesse taxonomique")
...
mapview(ibd_et_trend, xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution IBD")
mapview(ips_et_trend,xcol="longitude", ycol="latitude", zcol="trend", crs= 4326, grid=FALSE, layer.name="Evolution IPS")
...
-   réalisation d'un df, avec toutes les combinaisons de mois-début et mois-fin pour chaque paramètre (soit 78 combinaisons possibles).
-   df avec les colonnes suivantes : code_station_hydrobio, code_paramètre, annee, mois.
-   calcule des moyennes mensuelles intra paramètres physico-chimiques au niveau de chaque stations.
...
- l'I2M2 est davantage corrélé à l'ASPT, le polyvoltinisme et l'ovoviviparité qu'à l'indice de Shannon et la richesse taxonomique. Et celles-ci sont fortement corrélées entre elles. 

- l'I2M2 est corrélé à l'IBD et l'IPS. 

- deux sous-groupes de variables environnementales peuvent être extraits : la DBO5 et le taux de saturation vs le reste des paramètres. 
- corrélations les plus fortes (positives et négatives) : 
conductivité-MES; MES-O2 dissous, SaturO2-DBO5;NO2-NH4; NO2-NO3; NH4-NO3 (0,8); NO4-PO4;PO4-Turbidité (0,92); NO3-turbidité; NO2-PO4; MES-turbidité; MES-PO4; MES-NH4; MES-NO2, MES-NO3, O2dissous - N(n), O2dissous-Po4; SaturO2-N(n), Satur02-Corga; DBO5-Corga; DBO5- N(n), 
...
Les variables les mieux représentées sont les paramètres liés à l'oxygénation du milieu, les nutriments phosphorés et azotés. La variabilité des nitrates et de la température n'est pas "captée" par F1 et F2. On peut supposé que NO3 varie de manière indépendante des autres variables, ou porte une information non liée aux gradient F1 et F2. 
...
sjPlot::plot_model(model_normal_non_transformées, type = "pred",terms= ("O2dissous"), title = "Effets de l'O2 dissous  sur l'I2M2")
